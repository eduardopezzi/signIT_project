...
let verifier = keccak256(IPFSaddress).toString("hex");
...
contract("NewDocument", (accounts) => {
  ...
  it("1. NewDocument should be deployed", async () => {
    let Contract = await NewDocument.deployed(
      verifier,
      IPFSaddress,
      numberOfSigners
    );
  ...
  it("3. Verify function should recover signer address", async () => {
    ...
    let recover = await Contract.verify(signedHash, msg); //msg, sig.v, sig.r, sig.s);
    assert.equal(signer, recover, "Signer address match");
  });
  ...
  it("4.1. Document can be signed and event is emitted", async () => {
    ...
    let sigTX = await Contract.signDocument(
      // msg,
      // sig.v,
      // sig.r,
      // sig.s,
  ...
  it("7.1. Signature can be REVOKED by signer", async () => {
    ...
    let revokeSig = await Contract.revokeSignature(msg, signedHash, {
      from: signer,
    });
  ...
  it("7.2.Document can be REVOKED", async () => {
    ...
    let revokeSig2 = await Contract.revokeSignature(msg, signedHash, {
      from: signer2,
    });
  ...
  it("teste", async () => {
    ...
    // msg = EthUtil.toBuffer(msg);
    msg = EthUtil.hashPersonalMessage(new Buffer.from(msg));
    var sig = EthUtil.ecsign(msg, new Buffer.from(privateKeyIssuer, "hex"));
    var signedHash = EthUtil.toRpcSig(sig.v, sig.r, sig.s).toString("hex");
